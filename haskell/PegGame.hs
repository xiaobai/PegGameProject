...
solve n =
    do
        ...
        let listOfBoards = [[False] ++ [ True | _ <- [2..numOfPegs]]] ++ [ slice 0 (i-1) fullBoard ++ [False] ++ slice (i+1) numOfPegs fullBoard | i <- [1..(numOfPegs-1)]]
        let arr = [ recursiveSolve (listOfBoards !! i) n numOfPegs [(i,i)] | i <- [0..(quot (numOfPegs-1) 2)] ]
        getShortestList arr []
